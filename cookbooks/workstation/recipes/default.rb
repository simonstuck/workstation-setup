...
when "mac_os_x"
  ...
  wk['zips'].each do |pkg, data|
    remote_file "#{Chef::Config[:file_cache_path]}/#{pkg}.zip" do
      checksum  data['checksum']
      source data['url']
    end

    execute "unzip #{pkg}" do 
      command "unzip #{Chef::Config[:file_cache_path]}/#{pkg}.zip"
      cwd "/Applications"
      not_if { ::File.directory?("/Applications/#{pkg}.app") }
    end
  end
end
